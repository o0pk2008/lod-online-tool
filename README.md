# TwinScape LOD 在线转换工具

基于 Web 的 Gaussian Splat **多层 LOD 转换**工具：上传/选择 PLY·SOG 等模型，配置层数与保留率，**两段式**生成中间简化文件并合成 `lod-meta.json`，过程中用 PlayCanvas 三维预览。
...
- 参数 UI：LOD 层数、保留率、分块规模 `-C`
- 拖放导入、输出目录随输入文件名自动填充（`output/文件名`）
- 转换进度 SSE 推送；**刷新页面可恢复**配置与进度（服务进程需仍在运行）
- 三维预览：中间结果 / 分块流式 / 完成后 `lod-meta` 分层（`unified` 全局排序）
...
2. **参数**：设置 LOD 层数与各层保留率（L0 = 全量）  
3. **转换**：开始后查看任务列表；阶段 1 近邻搜索可能较久  
4. **预览**：右侧视口查看中间结果 / 分块 / 最终分层  
