...
- 参数 UI：LOD 层数、保留率、分块规模 `-C`（K×1024 个高斯）、最大空间跨度 `-X`（模型单位）；压缩模式为单一保留比例
- 导入后自动显示原模型，无需开始计算；可随时点击“预览输入模型 / 返回规划”。PLY / SOG 直接预览，其他输入格式先由 CLI 转为临时 PLY。
- 计算前分块规划：青色 box 与模型使用相同坐标变换；改变 `-C`、`-X`、层数及保留率自动更新，可关闭 box。后台线程最多采样 10 万个 L0 中心、绘制 2048 个 box。按最长轴中位数划分，并以保留率估算多层点数；这是空间规划估算，并非最终文件数量或精确边界（最终使用各层简化点与高斯尺寸）。
- “重置面板”清空路径、进度、任务列表、预览和对比导入，恢复默认参数，同时清除服务端最近任务快照，刷新不会恢复旧任务；磁盘输入与输出文件保留。运行中需先停止。
...
- LOD 调试：加载分层模型后按当前视角初始化切换距离。距离支持 0.1～1,000,000（对数滑条及数值输入），层间倍率为 1.2～8，默认 3，与固定的 PlayCanvas 2.21.3 引擎一致。可点击“按当前视角匹配距离”恢复自动模式；单层模式禁用距离控件。面板显示经视野角度修正的分块距离、目标层分布和各级阈值，实际显示以着色为准（异步加载可能滞后）。距离取相机到每个分块包围盒的最近距离，并非到模型中心；LOD 分级切换不提供透明渐变。
